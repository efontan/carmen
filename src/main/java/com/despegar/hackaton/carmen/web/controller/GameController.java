...
public class GameController {
    ...
    public ModelAndView index(HttpRequestContext context, HttpServletRequest request) throws Exception {
    ...
    public ResponseEntity<Object> getStatus(HttpRequestContext context, HttpServletRequest request,
        @PathVariable("token") String token) {
        // TODO
        GameSession gameSession = this.getSessionService().getSession(request).getGameSessionByToken(token);
        Status status = gameSession.getStatus();
        return new ResponseEntity<Object>(new Response<Status>(ResponseStatus.SUCCESS,status), HttpStatus.OK);
    }

    @RequestMapping(value = "/flight/travel/{from}/{to}/{token}", method = RequestMethod.GET)
    public ResponseEntity<Object> flightTravel(HttpRequestContext context, HttpServletRequest request,
        @PathVariable("from") String from, @PathVariable("to") String to, @PathVariable("token") String token) {
        // TODO

        TravelResponseDTO travelResponseDTO = new TravelResponseDTO();
        return new ResponseEntity<Object>(new Response<TravelResponseDTO>(ResponseStatus.SUCCESS, travelResponseDTO), HttpStatus.OK);
    }

    @RequestMapping(value = "/hotel/confirm/{hotelId}/{token}", method = RequestMethod.POST)
    public ResponseEntity<Object> hotelConfirm(HttpRequestContext context, HttpServletRequest request,
        @PathVariable("hotelId") Long hotelId, @PathVariable("token") String token) {
        // TODO

        new HotelConfirmDTO();
        return new ResponseEntity<Object>(new Response<Object>(ResponseStatus.SUCCESS,null), HttpStatus.OK);
    }
